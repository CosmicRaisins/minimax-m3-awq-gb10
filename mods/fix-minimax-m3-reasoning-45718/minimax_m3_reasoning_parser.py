...
class MiniMaxM3ReasoningParser(BaseThinkingReasoningParser):
    ...
    def _looks_like_rendered_prompt(self, token_ids: Sequence[int]) -> bool:
        special_ids = set(getattr(self.model_tokenizer, "all_special_ids", []) or [])
        special_ids.discard(self.start_token_id)
        special_ids.discard(self.end_token_id)
        return any(token_id in special_ids for token_id in token_ids)
    ...
    def is_reasoning_end(self, input_ids: Sequence[int]) -> bool:
        # The adaptive chat template includes literal marker examples inside
        # system instructions. vLLM calls this once with prompt token IDs before
        # generation; treating those examples as generated output disables the
        # streaming parser and leaks <mm:think> into content.
        if self._looks_like_rendered_prompt(input_ids):
            return False
